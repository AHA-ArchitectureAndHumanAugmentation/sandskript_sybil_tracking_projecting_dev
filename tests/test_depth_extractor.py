...
class TestGroovesFromDepth:
    ...
    def test_carved_groove_is_detected(self, depth_with_groove, flat_depth):
        out = grooves_from_depth(depth_with_groove, reference=flat_depth)
        assert out.max() == 255
        # The detected centreline should run along the carved row (y≈240).
        ys = np.where(out > 0)[0]
        assert 230 <= ys.mean() <= 250

    def test_groove_feeds_path_extractor(self, depth_with_groove, flat_depth):
        out = grooves_from_depth(depth_with_groove, reference=flat_depth)
        extracted = extract_from_edges(out, min_contour_pixels=20)
        assert extracted.total_strokes >= 1
        assert extracted.total_points > 0

    def test_skeleton_is_thinner_than_raw_mask(self, depth_with_groove, flat_depth):
        thin = grooves_from_depth(depth_with_groove, skeleton=True, reference=flat_depth)
        thick = grooves_from_depth(depth_with_groove, skeleton=False, reference=flat_depth)
        assert (thin > 0).sum() <= (thick > 0).sum()
        assert (thick > 0).sum() > 0

    def test_grooves_and_mask_matches_individual_calls(self, depth_with_groove, flat_depth):
        mask, skel = grooves_and_mask(depth_with_groove, reference=flat_depth)
        # mask == the thick mask; skel == its skeleton.
        assert (mask == groove_mask(depth_with_groove, reference=flat_depth)).all()
        assert (skel == grooves_from_depth(depth_with_groove, skeleton=True, reference=flat_depth)).all()
        assert (skel > 0).sum() <= (mask > 0).sum()

    def test_higher_threshold_rejects_shallow_groove(self, depth_with_groove, flat_depth):
...
# ─────────────────────────────────────────────────────────────────────────────
# Relative-depth detection
# ─────────────────────────────────────────────────────────────────────────────

class TestRelativeDetection:
    """Detection subtracts a reference frame of empty sand."""

    def test_detects_groove_against_reference(self, depth_with_groove, flat_depth):
        params = DepthGrooveParams(groove_depth_mm=1.0)
        out = grooves_from_depth(depth_with_groove, params=params, reference=flat_depth)
        assert out.max() == 255
        ys = np.where(out > 0)[0]
        assert 230 <= ys.mean() <= 250

    def test_no_groove_when_current_matches_reference(self, flat_depth):
        out = grooves_from_depth(flat_depth, reference=flat_depth)
        assert int(out.sum()) == 0

    def test_no_groove_without_reference(self, depth_with_groove):
        out = grooves_from_depth(depth_with_groove, reference=None)
        assert int(out.sum()) == 0

    def test_cancels_tilted_camera(self):
...
class TestProcessDepth:
    ...
    def test_full_frame(self, depth_with_groove, flat_depth):
        proc = process_depth(depth_with_groove, None, Crop(), DepthGrooveParams(),
                             reference=flat_depth)
    ...
    def test_crop_shifts_origin_and_shrinks_grooves(self, depth_with_groove, flat_depth):
...
class TestDepthGrooveParams:
    ...
    def test_unknown_keys_are_ignored(self):
        # Old presets may contain removed keys (detect, ref_strength, etc.);
        # parsing them must not raise.
        p = DepthGrooveParams.from_dict({
            "detect": "band", "ref_strength": 1.0,
            "detrend_sigma_px": 50, "band_center_mm": 5.0,
            "groove_depth_mm": 2.0,
        })
...
class TestNaturalGrooveRejection:
    ...
    def test_reference_subtraction_cancels_preexisting_groove(self):
        ...
        # Without reference: detection is inert.
        no_ref = grooves_from_depth(cur)
        assert int(no_ref.sum()) == 0

        # With reference subtraction: the natural (y=100) groove cancels.
        with_ref = grooves_from_depth(cur, reference=ref)
    ...
    def test_min_length_drops_short_grooves(self):
        ...
        base = grooves_from_depth(d, reference=ref)
        assert _has_row(base, 240) and _has_row(base, 100)

        p = DepthGrooveParams(min_length_mm=50.0)        # 50 mm = 100 px
        filt = grooves_from_depth(d, params=p, mm_per_px=mm_per_px, reference=ref)
    ...
    def test_min_mean_depth_drops_shallow_grooves(self):
        ...
        base = grooves_from_depth(d, params=p_all, reference=ref)
        assert _has_row(base, 240) and _has_row(base, 100)

        p = DepthGrooveParams(groove_depth_mm=1.0, min_mean_depth_mm=4.0)
        filt = grooves_from_depth(d, params=p, reference=ref)
    ...
    def test_width_band_drops_thin_grooves(self):
        ...
        filt = grooves_from_depth(d, params=p, mm_per_px=0.5, reference=ref)
    ...
    def test_from_dict_parses_and_clamps_new_keys(self):
        p = DepthGrooveParams.from_dict({
            "min_length_mm": -5, "min_width_mm": 100,
            "min_mean_depth_mm": 3, "ignore_closer_mm": -10,
        })
