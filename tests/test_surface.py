...
class TestLoadAndPayload:
    ...
    def test_mesh_payload_corners_always_empty(self):
        # Touch-off registration is a robot feature this repo doesn't have
        # (no robot ever connects here — see main.py's _NO_ROBOT_MSG). The
        # key stays in the payload for the browser's corner-marker code, but
        # it never has candidates to show.
        model = _flat_plane()
        payload = model.mesh_payload()
        assert payload["corners"] == []
        assert model.corner_points() == []
...
class TestSurfaceScene:
    ...
    def test_a_single_pose_moves_the_whole_assembly_rigidly(self):
        # No corner-registration solve any more (robot feature, unavailable
        # in this repo), but the underlying guarantee it relied on still
        # holds: one SurfacePose applied to the union moves every part
        # together, so the gap between them is unchanged after placement.
        scene = SurfaceScene([_plane_at(0.0, 0.0, "a"), _plane_at(1.0, 0.0, "b")])
        assert scene.corner_points() == []
