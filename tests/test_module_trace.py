...
class TestChain:
    ...
    def test_known_action_lists_its_modules(self):
        assert chain("save") == "path_export.py → zmq_bridge.py"
    ...
    def test_extra_modules_are_appended(self):
        out = chain("generate", extra=("surface",))
        assert out == "depth_extractor.py → path_extractor.py → surface.py"

    def test_extra_does_not_duplicate_an_existing_module(self):
        assert chain("save", extra=("path_export",)) == "path_export.py → zmq_bridge.py"
...
class TestLog:
    ...
    def test_prints_message_then_chain(self, capsys, monkeypatch):
        ...
        assert out[1] == "  └ path_export.py → zmq_bridge.py"
