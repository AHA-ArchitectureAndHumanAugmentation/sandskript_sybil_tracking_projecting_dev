...
GOFA_REACH_M sphere around the robot base and outside a thin GOFA_MIN_REACH_M
...
from config import GOFA_REACH_M, GOFA_MIN_REACH_M
...
def reach_flags(strokes: list[list[list[float]]]) -> tuple[list[list[int]], int, int]:
    ...
    for stroke in strokes:
        ...
        for p in stroke:
            ...
            bad = int(r > GOFA_REACH_M or r_xy < GOFA_MIN_REACH_M)
            f.append(bad)
            n_out += bad
            n_total += 1
        flags.append(f)
