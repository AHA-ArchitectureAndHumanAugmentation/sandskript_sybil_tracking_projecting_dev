...
# ── Shared state ──────────────────────────────────────────────────────────────
# Repo 1 (this app) never drives a robot — the "Connect"/"Run"/"Register
# Corner" controls below are stubs. Robot execution belongs to repo 2
# (sandskript_sybil_rrc_dev), which receives a saved bundle over ZeroMQ
# (zmq_bridge.send_path_to_charlotte) and runs it. See surfaces/README.md and
# sandskript_sybil_rrc_dev/documentation/README_pipeline.md for that half.
shared_state: dict = {
    "robot_connected":     False,   # always False here — no robot connection exists in this repo
...
_NO_ROBOT_MSG = (
    "Robot execution is not available in this repo — it runs on "
    "sandskript_sybil_rrc_dev, which receives a saved bundle over ZeroMQ."
)


# ── Robot connection callbacks (stubs — see _NO_ROBOT_MSG) ────────────────────
async def on_robot_connect(ip: str, ws) -> None:
    await server.send_connection_result(ws, False, _NO_ROBOT_MSG)
    module_trace.log("connect", f"[connect] refused — {_NO_ROBOT_MSG}")
...
async def on_simulate_workspace() -> None:
    ...
    tested without a real target surface. There is no local Run in this repo
    either way — see _NO_ROBOT_MSG.
    """
...
async def on_register_freedrive(ws, params: dict) -> None:
    """Touch-off registration is a robot feature — see _NO_ROBOT_MSG."""
    await server.send_register_result(ws, False, error=_NO_ROBOT_MSG)
    module_trace.log("register", f"[register] refused — {_NO_ROBOT_MSG}")
...
async def on_generate_path(ws, params: dict) -> None:
    ...
    await server.send_capture_result(
        ...
        # client-side when the exec-bar Offset/Safety inputs change. No
        # reach numbers here — reach/safety is repo 2's job (its own
        # SPHERE_CENTER/SAFETY_RADIUS), not duplicated in this repo.
        exec_viz={
...
async def on_run(ws, params: dict | None = None) -> None:
    """Running a path on a real robot is not available in this repo — save
    the bundle instead (on_save_path), which sends it to repo 2 over ZeroMQ."""
    await server.send_capture_result(ws, False, error=_NO_ROBOT_MSG)
    module_trace.log("run", f"[run] refused — {_NO_ROBOT_MSG}")


async def on_cancel(ws) -> None:
    """No local execution to cancel — see on_run."""
    module_trace.log("cancel", "[cancel] no-op — nothing runs locally in this repo")
...
async def _participant_pipeline() -> None:
    ...
    try:
        ...
        # ── Actuating: save the bundle. No local robot to run it on — saving
        # is what sends it onward (on_save_path -> zmq_bridge, if a tile_id
        # has arrived from sandskript_sybil_rrc_dev).
        ...
        automation.finish(
            "Done — path saved and sent to the robot pipeline." if tile_id is not None
            else "Path saved — no tile announced yet, not sent.")
    except Exception as exc:
